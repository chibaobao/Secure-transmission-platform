...
int KeyMngsvr_DBOp_WriteStatus(void *dbhdl, NodeSHMInfo *pNodeInfo) 
{
    int             ret = 0;
    char            mysql[2048] = {0};
    ...
    if (ret != 0)
    {
     ...
     //组织sql语句(clientid, serverid, keyid, createtime, state=1, seckey)
    sprintf(mysql, "update SECMNG.SECKYEINFO set state=%d where keyid=%d", pNodeInfo->status, pNodeInfo->seckeyid );
