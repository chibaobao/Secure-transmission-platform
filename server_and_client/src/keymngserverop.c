...
int MngServer_Revoke(MngServer_Info *svrInfo, MsgKey_Req *msgkeyReq, unsigned char **outData, int *datalen)
{
	...
	if (ret != 0)
	{
		KeyMng_Log(__FILE__, __LINE__,KeyMngLevel[4], ret,"func KeyMng_ShmRead() err，秘钥注销时服务器端读取共享内存失败");
		return ret;
	}
	...
	if (ret != 0)
	{
		KeyMng_Log(__FILE__, __LINE__,KeyMngLevel[4], ret,"func KeyMng_ShmWrite() err，秘钥注销时服务器端写共享内存失败");
		return ret;
	}
	...
	//往数据库中写
	ret = KeyMngsvr_DBOp_WriteStatus(handle, &nodeShmInfo) ;
	if (ret != 0)
	{
		KeyMng_Log(__FILE__, __LINE__,KeyMngLevel[4], ret,"func KeyMngsvr_DBOp_WriteSecKey() err");
		goto End;
