...
int MngClient_Check(MngClient_Info *pCltInfo)
{
	...
	if (iMsgKeyResType != ID_MsgKey_Res)
	{
		ret = MngClt_ParamErr;
		//关键步骤:一定要把出错的场景给打印日志  非常重要
		KeyMng_Log(__FILE__, __LINE__,KeyMngLevel[4], ret, "keymngclient校验 服务器的应答报文错误:iMsgKeyResType:%d", iMsgKeyResType);
		goto End;
	}
	printf("seckeyid:%d \n", pMsgKeyRes->seckeyid);
...
int MngClient_Revoke(MngClient_Info *pCltInfo)
{
	...
	if (pMsgKeyRes->rv != 0)
	{
		ret = pMsgKeyRes->rv;
		//关键步骤:一定要把出错的场景给打印日志  非常重要
		KeyMng_Log(__FILE__, __LINE__,KeyMngLevel[4], ret, "服务器端注销密钥失败rv:%d", pMsgKeyRes->rv);
		goto End;
	}
	else
	{
